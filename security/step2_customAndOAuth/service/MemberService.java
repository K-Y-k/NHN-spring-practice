package com.nhnacademy.restcontrollerpractice.security.step2_customAndOAuth.service;
...
public class MemberService {
    ...
    public Member saveMember(MemberCreateCommand memberCreateCommand) {
        //TODO
        // 중복체크 Error
        if (redisTemplate.opsForHash().hasKey(HASH_NAME, memberCreateCommand.getId())) {
            Object o = redisTemplate.opsForHash().get(HASH_NAME, memberCreateCommand.getId());
            return objectMapper.convertValue(o, Member.class);
        }

        Member createMember = new Member(memberCreateCommand.getId(), memberCreateCommand.getPassword(), memberCreateCommand.getName(), memberCreateCommand.getRole());
